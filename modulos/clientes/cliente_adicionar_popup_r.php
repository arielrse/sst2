...
$cfax = $_POST["cfax"];
$cobs = $_POST['cobs'];
$dato=mysqli_fetch_array(mysqli_query($conexion, "select ADDTIME(now(),ajuste) FROM ajuste_hora_server LIMIT 1"));
$fecha_registro=$dato[0];

$dato=mysqli_fetch_array(mysqli_query($conexion, "SELECT valor FROM secuencias WHERE id_secuencia='clientes';"));
$dato=mysqli_fetch_array(mysqli_query($conexion, "SELECT incrementar( '".$dato[0]."',1)"));
$nro_=$dato[0];
mysqli_query($conexion, "UPDATE secuencias SET valor='".$nro_."' WHERE id_secuencia='clientes';");
...
{
?>
<div align="center">Acci�n exitosa!<br />
...
else echo"Ocurrio un error: ".mysqli_error();
