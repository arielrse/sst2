...
$enviar_mail = $_POST["enviar_mail"];
	...
	$dato=mysqli_fetch_array(mysqli_query($conexion, "select id_st_proyecto,id_item,id_usuario,date_format(fecha,'%d/%m/%Y') AS fecha FROM st_cronograma_informes_".$pro_key." WHERE id_st_cronograma_informes_".$pro_key."='".$id_st_cronograma_informes."'"));
...
$dato=mysqli_fetch_array($resultado);
 ...
 $ubicacion=$dato[4];
	...
	$dato=mysqli_fetch_array(mysqli_query($conexion, "SELECT f.mail as m1,CONCAT(f.nombre,' ',f.ap_pat) as n1, s.mail as m2, CONCAT(s.nombre,' ',s.ap_pat) as n2, t.mail as m3 FROM usuarios f, usuarios s, usuarios t WHERE f.id='".$tecnico."' AND s.id='".$tecnico_anterior."' AND t.id='".$id_user."'"));
	$mail_tecnico=$dato[0];
	$name_tecnico=$dato[1];
	...
	$cuerpo_send="Sr. ".$name_tecnico." se le ha reformulado la fecha de la visita t�cnica del ".$fecha." al ".$_POST['fecha_nuevo']." <HR>".$cuerpo; 
	...
	$cuerpo_send="Sr. ".$name_tecnico." se le ha asignado la siguiente visita t�cnica el ".$_POST['fecha_nuevo']." <HR>".$cuerpo;
	mail($mail_tecnico,$asunto,$cuerpo_send,$headers);	
	}			
	}
