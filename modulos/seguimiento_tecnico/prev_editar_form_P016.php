...
JOIN formulario_p016 h ON ev.idevento = h.idevento
JOIN p016_formulario f ON h.id = f.id
...
$resultQuery = mysqli_query($conexion, "SELECT * FROM formulario_p016 p WHERE p.id = ".$idformtto);
...
<form name="amper" method="post" action="<?=$link_modulo_r?>" onSubmit=" return VerifyOne ();">
	<input type="hidden" name="path" value="prev_nuevo_form_P016_r.php" />
...
<table width="900" align="center" class="table2">
	<caption>FORMULARIO DE MANTENIIENTO PREVENTIVO<br>EQUIPOS SATELITALES</caption>
</table>
...
<table width="900" align="center" class="table2">
	...
	<tr>
	  <th width="25%">Se tiene equipo satelital?</th>
	  <td width="75%">
	...
	<tr>
		...
		<td width="75%"><input class='Text_left' name="p03" type="text" size="30" value="<?=$p03?>" /></td>
	</tr>

	<tr>
		<th width="25%">Modelo</th>
		<td width="75%"><input class='Text_left' name="p04" type="text" size="30" value="<?=$p04?>" /></td>
	</tr>

	<tr>
		<th width="25%">Rx Signal(Eb/No) dB</th>
		<td width="75%"><input class='Text_left' name="p05" type="text" size="30" value="<?=$p05?>" /></td>
	</tr>

	<tr>
		<th width="25%">Nivel enlace (Downstream) dB</th>
		<td width="75%"><input class='Text_left' name="p06" type="text" size="30" value="<?=$p06?>" /></td>
	</tr>

	<tr>
		<th width="25%">Nivel enlace (UPsstream) dB</th>
		<td width="75%"><input class='Text_left' name="p07" type="text" size="30" value="<?=$p07?>" /></td>
	</tr>

	<tr>
		<th width="25%">Capacidad</th>
		<td width="75%"><input class='Text_left' name="p08" type="text" size="30" value="<?=$p08?>" /></td>
	</tr>

	<tr>
		<th width="25%">Ancho de banda utilizado (Kbps)</th>
		<td width="75%"><input class='Text_left' name="p09" type="text" size="30" value="<?=$p09?>" /></td>
	</tr>

	<tr>
		<th width="25%">Frecuencia de trabajo</th>
		<td width="75%"><input class='Text_left' name="p10" type="text" size="30" value="<?=$p10?>" /></td>
	</tr>

	<tr>
		<th width="25%">Configuraci�n</th>
		<td width="75%">
			<select name="p11" class="selectb">
				<?
...
<table  width="900" align="center" class="table2">
	...
	<?
	$text =	"Verificar la instalaci�n del Equipo;;|".
		...
		for ($i = 0; $i < sizeof($subarray); $i++) {
			switch($i){
				case 0: $result .= "<td>$subarray[$i]</td>";
						$result .= "<input name='p12$a' id='$a$b' type='hidden' value='$subarray[$i]'>";
						break;
				case 1: $result .= "<td align='center'><input type='checkbox' name='p12$a' value='$a'";
						if($subarray[$i] == $a) $result .= "checked";
						$result .= " ></td>";
						break;
				default: $result .= "<td align='center'><input class='Text_left' type='text' name='p12$a' value='$subarray[$i]' size='20'/></td>";
			}
			$a++;
		}
		$result .= "</tr>";
	...
	echo $result;
	?>
	<input type="hidden" name="size12" value="<?=$a?>" />
</table>
	<br />
<table  width="900" align="center" class="table2">
		...
		foreach ($arrays as $array){
			...
			for ($i = 0; $i < sizeof($subarray); $i++) {
				if($i == 0){
					...
					$result .= "<input name='p13$a' type='hidden' value='$subarray[$i]'>";
				}else{
					$result .= "<td align='center'><input class='Text_left' type='text' name='p13$a' size='20' value='$subarray[$i]' /></td>";
				}
				$a++;
			}
			$result .= "</tr>";
