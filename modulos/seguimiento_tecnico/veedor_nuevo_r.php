...
$resultado=mysqli_query($conexion, "INSERT INTO st_personal_veedor(id_st_personal_veedor,id_cliente,nombre_completo,cuenta,clave,correo,descripcion) VALUES('','$id_cliente','$nombre_completo','$cuenta','$clave','$correo','$descripcion');");
		if($resultado)	{
		
		$dato=mysqli_fetch_array(mysqli_query($conexion, "SELECT mail FROM usuarios WHERE id='$id_user'"));
		$remitente=$dato[0];

		$dato=mysqli_fetch_array(mysqli_query($conexion, "SELECT razon_social FROM clientes WHERE id='$id_cliente'"));
	    $razon_social=$dato[0];
