...
$dato_t=mysqli_fetch_array(mysqli_query($conexion, "SELECT departamento,producto,marca,caracteristicas,ubicacion,sn,idestacion FROM st_trabajos WHERE id_item='".$dato['id_item']."'"));

$estacion = $dato_t['ubicacion'];
if (!is_null($dato_t['idestacion'])){
    $dato_e = mysqli_fetch_array(mysqli_query($conexion, "SELECT codigo, nombre FROM estacion WHERE idestacion = ".$dato_t['idestacion']));
...
<table width="100%" class="table2">
    ...
    <table width="100%" class="table2">
        ...
        <tr>
            ...
            <td width="50%">
                <?
                $output = '';
                $resultado=mysqli_query($conexion, "SELECT t.id_st_ticket, t.ticket, t.estacion, t.fecha_inicio_rif 
    ...
    <table width="100%" class="table2">
        ...
        <tr>
            ...
            <?
            $output_est = '';
            $resultado = mysqli_query($conexion, "SELECT ea.idestacion, e.nombre, e.codigo, e.provicia 
...
if( checkField( document.amper.detalles, isName, false)
    ...
    */
    ...
    if(confirm("Esta Guardando esta informaci�n y Pasando al Paso 2 de 3.")){return true;}
    else {return false;}
    }
else {return false;}   	
}
