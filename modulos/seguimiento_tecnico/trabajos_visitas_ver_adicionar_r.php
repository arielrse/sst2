...
$id_st_proyecto = $_POST["id_st_proyecto"];
	...
	$dato_st=mysqli_fetch_array(mysqli_query($conexion, "SELECT MAX(periodo) AS periodo FROM st_cronograma_informes_".$pro_key." WHERE id_item='".$id_item."'"));
    $periodo=$dato_st['periodo']+1;
	
	if($hora_p!="" && $hora_p!=NULL)
	    mysqli_query($conexion, "INSERT INTO st_cronograma_informes_".$pro_key."(id_st_proyecto,id_cliente,id_usuario,detalles,id_item,fecha,hora_programada,periodo,p1) VALUES('$id_st_proyecto','$id_cliente','$tecnico','$detalles',$id_item,'$fecha','$hora_p','$periodo','$fecha_not')");
	else
	    mysqli_query($conexion, "INSERT INTO st_cronograma_informes_".$pro_key."(id_st_proyecto,id_cliente,id_usuario,detalles,id_item,fecha,periodo,p1) VALUES('$id_st_proyecto','$id_cliente','$tecnico','$detalles',$id_item,'$fecha','$periodo','$fecha_not')");
