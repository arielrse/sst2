...
<?
	$resultado = mysqli_query($conexion, "
		...
		SELECT a.id, a.codigo, a.nombre, a.titulo, a.idevento
			FROM formulario_mtto a
			WHERE a.idevento = $idevento 
		union
		SELECT p.id, p.codigo, p.nombre, p.titulo, p.idevento
			FROM formulario_mtto_n p
			...
			<tr onmouseover="setPointer(this, '#DADADA')" onmouseout="setPointer(this, '<?=$rowt?>')">
				...
				<td style="background:<?=$rowt?>">
					<div align="center">
						...
						<?php if($codigo=='P001' || $codigo=='P002' ||               $codigo=='P001v' || $codigo=='P002v' ||
								 ...
								 $codigo=='P013n' || $codigo=='P019' 
								 || $codigo=='P019v' 

						) 	{ ?>
						<a class="enlaceboton"
						   href="../../pdf/pdf_st_<?=$codigo?>.php?cod=<?=base64_encode($codigo)?>&idformtto=<?=base64_encode($idformtto)?>&idevento=<?=base64_encode($idevento)?>"
						...
						<a title="Eliminar Formulario"
						   class="enlaceboton"
						   href="<?=$link_modulo_r?>?path=eliminar_formulario_mtto.php&idform=<?=base64_encode($idformtto)?>&codigo=<?=$codigo?>&params=<?=base64_encode($params)?>&idevento=<?=$idevento?>">
