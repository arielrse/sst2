...
$resultQuery = mysqli_query($conexion, "SELECT * FROM formulario_p017 p WHERE p.id = ".$idformtto);
		...
		for ($i = 0; $i < sizeof($subarray); $i++) {
			switch($i){
				case 0: $result .= "<td>$subarray[$i]</td>";
						...
						break;
				case 1: $result .= "<td align='center'><input type='checkbox' name='p06$a' value='$a'";
...
<table  width="900" align="center" class="table2">
		...
		foreach ($arrays as $array){
			...
			for ($i = 0; $i < sizeof($subarray); $i++) {
				if($i == 0){
					$result .= "<td align='center'><input class='Text_left' type='text' name='p07$a' size='30' value='$subarray[$i]' /></td>";
				}else{
					$result .= "<td align='center'><input class='Text_left' type='text' name='p07$a' size='20' value='$subarray[$i]' /></td>";
				}
				$a++;
			}
			$result .= "</tr>";
