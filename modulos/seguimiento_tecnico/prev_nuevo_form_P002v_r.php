...
$quer= mysqli_query($conexion, "SELECT MAX(id) FROM formulario_p002v");
$rowf= mysqli_fetch_row($quer);
 $id = trim($rowf[0])+1;
...
$query= mysqli_query($conexion, "SELECT MAX(id) FROM formulario_p002v");
$row = mysqli_fetch_row($query);
$idm = $idm = trim($row[0]);;

$Hoy = date("Y-m-d H:i:s");
$srr = mysqli_query($conexion, "INSERT INTO p002v_formulario VALUES ('$idm','$ideventos','$responsable','$pss10','$pss11','$serenos','$observacion','$fechamantenimiento','$Hoy')");


	$res = mysqli_query($conexion, "INSERT INTO formulario_mtto_n VALUES(22,'$idm','P002v','Formulario Mtto. Preventivo Estructura','$tete','$ideventos')");
 }else

  $sql =mysqli_query($conexion, "UPDATE p002v_formulario SET responsable = '$responsable', fechamantenimiento = '$fechamantenimiento' WHERE id = '$idformtto'");
  
  
    $sql =mysqli_query($conexion, "UPDATE formulario_mtto_n SET titulo = '$tete' WHERE id = '$idformtto'");
