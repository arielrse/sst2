...
$datox=mysqli_fetch_array(mysqli_query($conexion, "SELECT concat(nombre, ' ', ap_pat) as usuario FROM usuarios WHERE id='".$dato['id_usuario']."'"));
...
<td width="35%" align="right"><span class="medium">INF:</span><span class="medium azul"> F001<?=str_pad($id_st_cronograma_informes, 4, "0", STR_PAD_LEFT);?></span></td>
</tr>
<tr>
...
<?php 
	$resultado=mysqli_query($conexion, "SELECT titulo,imagen,item FROM st_cronograma_informes_".$pro_key."_archivos WHERE id_st_cronograma_informes_".$pro_key."='".$id_st_cronograma_informes."' ORDER BY item ASC");
...
<?php if($adm){ ?>
<a href="<?=$link_modulo_r?>?path=adjuntar_archivo.php&id_item=<?=$dato['id_item']?>&id_st_cronograma_informes=<?=$id_st_cronograma_informes?>&volver=3" class="enlace_s_menu" onclick="return GB_showCenter('ADICIONAR ARCHIVO', this.href,182, 700)">
	<img src="../../img/add_archivos.gif" alt="ADICIONAR ARCHIVO" vspace="0" border="0" align="absbottom"> <B>Adjuntar Archivo al Informe</B>
</a>
<?php } ?>
</div>
</td>
...
<tr><td width="50%"><span class="title6">T�cnico responsable:</span>
		  <?
		$datox=mysqli_fetch_array(mysqli_query($conexion, "SELECT u.mail,CONCAT(u.nombre,' ',u.ap_pat) AS tecnico FROM st_cronograma_informes_".$pro_key." i INNER JOIN usuarios u ON i.id_usuario=u.id WHERE i.id_st_cronograma_informes_".$pro_key."='".$id_st_cronograma_informes."'"));
	$bcc=$datox['mail'];
	?>
...
<tr><td width="50%"><span class="title6">Destinatarios Cliente:</span>
	...
	$consulta="SELECT CONCAT(nombre,' ',ap_pat) AS usuario,mail FROM usuarios WHERE nivel='3'";
	$resultado=mysqli_query($conexion, $consulta);
	$k=1;
	$filas=mysqli_num_rows($resultado);
	if($filas!=0)
	{  
		while($datoc=mysqli_fetch_array($resultado))
 		{
		?>
		...
		<BR /><label id="label<?=$k?>" class="marcar_check"><input name="notificar[]" type="checkbox" value="<?=$datoc['mail']?>" id="check_<?=$k?>" onclick="marcar_check(this,'label<?=$k?>')" />
...
<tr>
<td height="20" class="cafe">Este mensaje no forma parte del Informe, solo es informaci�n para fines internos:</td>
</tr>
<tr>
<td height="20"><textarea name="obs_int" class="resizable" style="width: 590px; height: 50px;" id="obs_int"><?=$dato['obs_int']?></textarea></td>
</tr>
<tr><td>
<span class="title6">Enviar Notificaci�n de Revisi�n a:</span>
<?
$consulta="SELECT CONCAT(nombre,' ',ap_pat) AS usuario,mail FROM usuarios WHERE nivel='1'";
$resultado=mysqli_query($conexion, $consulta);
$k=1;
...
<tr>
  <td class="marco">
	<? echo" <a class=\"enlaceboton\" href='../../html/html_st_".$pro_key.".php?id_st_cronograma_informes=".base64_encode($id_st_cronograma_informes)."&id_item=".base64_encode($dato['id_item'])."' onClick=\"openNewWindowhtml( this, '800', '590' );return false;\"><img src='../../img/c_html.gif' alt='Ver Informe en HTML' border=\"0\">Vista preliminar en HTML</a>"; ?>
  </td>
  <td class="marco"><? echo"<a class=\"enlaceboton\" href='../../pdf/pdf_st_".$pro_key.".php?id_st_cronograma_informes=".base64_encode($id_st_cronograma_informes)."&id_item=".base64_encode($dato['id_item'])."' onClick=\"openNewWindowhtml( this, '800', '590' );return false;\"><img src='../../img/imp.gif' alt='Ver Informe en PDF' border=\"0\">Vista Preliminar en PDF</a> ";
?></td>
  <?php if($adm){ ?>
  <td class="marco">
	<a class="enlaceboton" href="<?=$link_modulo?>?path=trabajos_informar_<?=$pro_key?>_2.php&id_st_cronograma_informes=<?=$id_st_cronograma_informes?>" target='_top'>
	<img src='../../img/change.gif' alt='Volver al Paso Anterior' border="0" align="absmiddle"> Volver Al Paso Anterior 
	</a>
	<?php } ?>
  </td>
</tr>
...
{ 
if(confirm("ESTA VALIDANDO ESTE FORMULARIO\n Verific� bien los datos antes de continuar?"))
{return true;}
else {return false;}   
