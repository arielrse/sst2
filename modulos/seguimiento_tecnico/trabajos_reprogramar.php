...
$resultado=mysqli_query($conexion, "SELECT periodo,id_usuario,date_format(fecha,'%d/%m/%Y') AS fecha,date_format(hora_programada,'%H:%i') AS hora FROM st_cronograma_informes_".$pro_key." WHERE id_st_cronograma_informes_".$pro_key." ='".$id_st_cronograma_informes."'");
...
if(document.amper.fecha_nuevo.value!=""){	
	if(confirm("Las datos Son validos:\nRevis� bien la coherencia de los datos antes de continuar?")){
	return true;
	}
	else{
	return false;
