...
$url=$web."/pdf/pdf_st_f001.php?id_st_cronograma_informes=".base64_encode($id_st_cronograma_informes)."&id_item=".base64_encode($id_item);
...
if($validar=="ENVIAR A REVISION"){
  ...
  //correo al tecnico
	$datox=mysqli_fetch_array(mysqli_query($conexion, "SELECT mail FROM usuarios WHERE id='".$id_user."'"));
	$bcc=$datox['mail'];
	$mail->AddAddress($bcc);
  //correo a los adm
	...
	$url=$web."/html/html_st_f001.php?id_st_cronograma_informes=".base64_encode($id_st_cronograma_informes)."&id_item=".base64_encode($id_item)."&archivos=no";
 ...
 //////----- echo "<hr>".$cuerpo;  			
		...
		if(!$mail->Send()) {
 		...
 		header("location: ".$link_modulo."?path=trabajos_ver.php&nro=".base64_encode($id_st_proyecto));	
	}
...
$condicion_final = $_POST["condicion_final"];
  ...
  $mail->AddReplyTo($remitente, $nombre_remitente);
    //correo al tecnico
	$datox=mysqli_fetch_array(mysqli_query($conexion, "SELECT u.mail FROM st_cronograma_informes_".$pro_key." i INNER JOIN usuarios u ON i.id_usuario=u.id WHERE i.id_st_cronograma_informes_".$pro_key."='".$id_st_cronograma_informes."'"));
	$bcc=$datox['mail'];
...
//////-----  echo "<hr>".$cuerpo;  			
	...
	else echo "<center><b>DIMESAT SRL: Ocurrio un Error!</b><br><a href='javascript:history.back(1);'>[RETORNAR]</a>
	</center>";
