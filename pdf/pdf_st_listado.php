...
$pdf->ezText("<b>Declaraci�n del proyecto:</b>\n".translate($declaracion_proyecto),10);
...
$dato_st=mysqli_fetch_array(mysqli_query($conexion, "SELECT b.descripcion FROM st_trabajos a, parametrica b  WHERE a.id_item='".$dato[6]."' AND b.grupo='st' AND b.sub_grupo=a.producto"));
$st_cronograma_informes=$dato_st['descripcion'];
	 
	 $resultadox=mysqli_query($conexion, "SELECT date_format(fecha,'%d/%m/%y'),condicion_final FROM ".$st_cronograma_informes." WHERE id_item='".$dato[6]."' AND condicion_final<>'' ORDER BY fecha DESC limit 1");
	 $filasx=mysqli_num_rows($resultadox);
	 
	 if($filasx!=0)
	 { 
	  $b=mysqli_fetch_array($resultadox);
	  $bb1=$b[1];
	  $bb2=$b[0];
	 }
	 else { $bb1="Sin Reporte"; $bb2=" ";}
	 
	 $c=mysqli_fetch_array(mysqli_query($conexion, "SELECT count(id_".$st_cronograma_informes."),count(condicion_final) FROM ".$st_cronograma_informes." WHERE id_item='".$dato[6]."'"));
