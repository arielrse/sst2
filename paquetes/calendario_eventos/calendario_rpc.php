<?php
	...
	case 'listEvents':
		...
		$eventQuery = mysqli_query($conexion, "SELECT id_item, responsable, departamento, ubicacion, producto, marca, caracteristicas 
FROM v_st_cronograma
WHERE fecha=FROM_UNIXTIME($timeStamp + 3600,'%Y-%m-%d')", $conexion);
		if($eventQuery) {
			if(mysqli_num_rows($eventQuery) >0) {
				...
				for($i=0; $i<mysqli_num_rows($eventQuery); $i++) {
					...
					extract(mysqli_fetch_array($eventQuery), EXTR_PREFIX_ALL, 'e');
						...
						echo '<div style="background-color: '. $bgColor .'; margin-bottom: 4px; padding: 1px;" id="event_'.$e_id_item.'">
								<div>
									'.($i+1).': '. $e_responsable.' ['.$e_departamento.' - '.$e_ubicacion.'] | ['.$e_producto.' - '.$e_marca.' - '.$e_caracteristicas.']
								</div>
							</div>';
