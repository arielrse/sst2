...
$dato=mysqli_fetch_array(mysqli_query($conexion, "SELECT count(*) as contar FROM st_cronograma_informes a INNER JOIN st_trabajos b ON b.id_st_proyecto='$nro' AND a.id_item=b.id_item GROUP BY a.id_item order by contar DESC LIMIT 1"));
$periodos=$dato[0];
...
$hasta=$k;
	...
	$resultadoa=mysqli_query($conexion, "SELECT id_item FROM st_trabajos WHERE id_st_proyecto='$nro'");
	while($datoa=mysqli_fetch_array($resultadoa))
	 {
		$datob=mysqli_fetch_array(mysqli_query($conexion, "SELECT id_item,condicion_final,postm_condicion_final FROM st_cronograma_informes WHERE id_item=".$datoa['id_item']." ORDER BY fecha ASC LIMIT $desde,$hasta;")); 
...
<table width="100%" border="0">
  <tr>
	...
	<?
	$dato=mysqli_fetch_array(mysqli_query($conexion, "SELECT COUNT(*) FROM c_pagar_internas WHERE pagado = '0' AND pago = '0'"));	
	$pendiente1=$dato[0];
	$dato=mysqli_fetch_array(mysqli_query($conexion, "SELECT COUNT(*) FROM c_pagar_internas WHERE pagado = '0' AND pago = '1'"));	
	$pendiente2=$dato[0];	
	$dato=mysqli_fetch_array(mysqli_query($conexion, "SELECT COUNT(*) FROM ext_c_pagar WHERE	pagado = '0' AND pago = '0'"));	
	$pendiente3=$dato[0];	
	$dato=mysqli_fetch_array(mysqli_query($conexion, "SELECT COUNT(*) FROM ext_c_pagar WHERE	pagado = '0' AND pago = '1';"));	
	$pendiente4=$dato[0];	
	$dato=mysqli_fetch_array(mysqli_query($conexion, "SELECT COUNT(*) FROM solicitud_compra_int WHERE autorizado_por = '0' AND confirmar!='0' AND validacion='0'"));
	$pendiente5=$dato[0];
	$dato=mysqli_fetch_array(mysqli_query($conexion, "SELECT COUNT(*) FROM orden_compra WHERE autorizado_por='0'"));
	$pendiente6=$dato[0];		
	?>
	...
	<img src="../../img/billete_ico.gif" border="0" align="absbottom"> Tiene <b>(<?=$pendiente5?>)</b> SOLICITUDES DE COMPRA Interno para autorizaci�n</a><br>
	<a href="../ordenesdecompra/ver_ocn.php" class="enlaceboton" title="Ordenes de Compra">
	<img src="../../img/billete_ico.gif" border="0" align="absbottom" > Tiene <b>(<?=$pendiente6?>)</b> ORDENES DE COMPRA para autorizaci�n	</a><br>
	</fieldset></td>
...
<table width="100%" border="0">
<?
	$dato=mysqli_fetch_array(mysqli_query($conexion, "SELECT COUNT(*) FROM tareas_laboratorio WHERE estado_actual='0'"));	
	$pendiente1=$dato[0];
	$dato=mysqli_fetch_array(mysqli_query($conexion, "SELECT COUNT(*) FROM crm_propuesta WHERE estado='En espera'"));	
	$pendiente2=$dato[0];
	$dato=mysqli_fetch_array(mysqli_query($conexion, "SELECT COUNT(*) FROM crm_orden_venta WHERE estado='P'"));	
	$pendiente3=$dato[0];
?>
  <tr>
    ...
    <?=$pendiente3?>
    )</b> ORDENES DE VENTA pendientes de actualizaci�n de estado. </a>
    </fieldset>
      <?
	$dato=mysqli_fetch_array(mysqli_query($conexion, "SELECT COUNT(*)-1 FROM proyectos WHERE vigencia=0"));	
	$inf1=$dato[0];
	$dato=mysqli_fetch_array(mysqli_query($conexion, "SELECT COUNT(*) FROM proyectos WHERE vigencia=1"));	
	$inf2=$dato[0];
	$dato=mysqli_fetch_array(mysqli_query($conexion, "SELECT COUNT(*) FROM proyectos WHERE vigencia=0 AND id_user='$id_user'"));	
	$inf3=$dato[0];
	$dato=mysqli_fetch_array(mysqli_query($conexion, "SELECT COUNT(*) FROM proyectos WHERE vigencia=1 AND id_user='$id_user'"));	
	$inf4=$dato[0];
	?>
      <fieldset>
      ...
      <?
	$dato=mysqli_fetch_array(mysqli_query($conexion, "SELECT COUNT(*) FROM solicitud_compra_int WHERE confirmar='0' AND solicitado_por='$id_user'"));
	...
	?>
      ...
      )</b> Solicitud de Compra Interno Suyas Pendientes de Validaci�n y/o Autorizaci�n </a>
      </fieldset></td>
  </tr>
</table>
<fieldset>
	<legend class="title4">Proyectos Seguimiento T�cnico:</legend>	
	<? 
	$resultado=mysqli_query($conexion, "SELECT id_st_proyecto FROM st_proyecto");
	while($proyecto=mysqli_fetch_array($resultado))
	{		
	$nro=$proyecto['id_st_proyecto'];
	
	$dato=mysqli_fetch_array(mysqli_query($conexion, "SELECT c.razon_social FROM st_proyecto s INNER JOIN clientes c ON s.id_cliente=c.id WHERE s.id_st_proyecto='$nro'"));	
	$cliente=$dato[0];
	$dato=mysqli_fetch_array(mysqli_query($conexion, "SELECT count(*) FROM st_trabajos WHERE id_st_proyecto='$nro'"));	
	$trabajos=$dato[0];
	?>
...
<caption>RESUMEN DE TAREAS PROXIMAS EN SEGUIMIENTO TECNICO (Pr�ximos 15 d�as)</caption>
<tr bgcolor="#485765">
<th width="2%">N&deg;</th>
<th width="4%">DIA</th>
<th  width="7%">FECHA</th>			              
...
<?
$dato=mysqli_fetch_array(mysqli_query($conexion, "SELECT COUNT(estado) AS nro_contactos FROM contactos WHERE estado!='C' AND id_user='$id_user'"));
...
<?
$dato=mysqli_fetch_array(mysqli_query($conexion, "SELECT tc FROM tc WHERE moneda='dolar'"));
$tc=$dato[0];	 

$dato=mysqli_fetch_array(mysqli_query($conexion, "SELECT YEAR(mes_anterior) AS anio, MONTH(mes_anterior) AS mes FROM (SELECT DATE_SUB(NOW(),INTERVAL 1 MONTH) as mes_anterior) AS t1;"));
	$ant_anio=$dato['anio'];	 
	$ant_mes=$dato['mes'];
...
<?
	$dato=mysqli_fetch_array(mysqli_query($conexion, "SELECT COUNT(*) FROM proveedor"));	
	$inf1=$dato[0];
	$dato=mysqli_fetch_array(mysqli_query($conexion, "SELECT COUNT(*) FROM proveedor WHERE creador='$id_user'"));	
	$inf1_=$dato[0];
	$dato=mysqli_fetch_array(mysqli_query($conexion, "SELECT COUNT(*)-1 FROM clientes"));	
	$inf2=$dato[0];
	$dato=mysqli_fetch_array(mysqli_query($conexion, "SELECT COUNT(*) FROM clientes WHERE creador='$id_user'"));	
	$inf2_=$dato[0];
?>	
	<fieldset>
...
<fieldset>
	...
	<a href="../../../users/ver_usuarios_web.php?llave_de_acceso=asdfghjkl" class="enlaceboton" onClick="openNewWindow( this, '800', '600' );return false;"><img src="../../img/users.gif" alt="otros" width="16" height="16" border="0"> Administrar Usuarios Registrados en Data Center </a><a href="../../control/cambiar_contrasena.php" class="enlaceboton" onClick="openNewWindow( this, '300', '150' );return false;"></a><br>
		...
		<a href="<?=$web?>/manuales/correo_ampersystem.html" class="enlaceboton" onClick="openNewWindow( this, '800', '600' );return false;"><img src="../../img/help.gif" alt="Manual" width="17" height="17" border="0" align="absbottom"> Guia para acceder a correo electr�nico AMPERSYSTEM</a><BR>
